...
class Side_Bar:
    ...
    def display_text(
        ...
        render_text_rect.center = Vector2(
            self.sideBar_surface.get_size()[0] / 2, position_y
        )
    ...
    def display_task(
        ...
        material_text = self.small_font.render(
            "~ Materials ~", True, text_color, self.bg_color
        )
        ...
        for index, material_icon in enumerate(task.material_icons):
            position_x = (
                self.sideBar_surface.get_size()[0]
                / (len(task.material_icons) + 1)
                * (index + 1)
            ) - material_icon.get_size()[0] / 2
            if task.material_names[index] in grid.get_anvil_inventories()[0]:
                ...
                self.sideBar_surface.blit(
                    completed_material_icon, (position_x, position.y + spacing * 2)
                )
            else:
                self.sideBar_surface.blit(
                    material_icon, (position_x, position.y + spacing * 2)
                )

        if task_complete:
            complete_text = self.small_font.render(
                "Press SPACE to Complete", True, text_color, self.bg_color
            )
