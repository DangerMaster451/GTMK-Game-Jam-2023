...
class Game_Over_Screen:
    ...
    def render(self, score: int):
        ...
        render_text_rect = render_text.get_rect()
        render_text_rect.center = Vector2(self.surface.get_size()[0] / 2, 100)
        self.surface.blit(render_text, render_text_rect)

        render_text = self.small_font.render(str(score), True, (255, 255, 255), self.bg_color)
        render_text_rect = render_text.get_rect()
        render_text_rect.center = Vector2(self.surface.get_size()[0] / 2, 125)
        self.surface.blit(render_text, render_text_rect)

        render_text = self.normal_font.render("High Score", True, (255, 255, 255), self.bg_color)
        render_text_rect = render_text.get_rect()
        render_text_rect.center = Vector2(self.surface.get_size()[0] / 2, 200)
        self.surface.blit(render_text, render_text_rect)

        render_text = self.small_font.render(self.high_score, True, (255, 255, 255), self.bg_color)
        render_text_rect = render_text.get_rect()
        render_text_rect.center = Vector2(self.surface.get_size()[0] / 2, 225)
        self.surface.blit(render_text, render_text_rect)

        render_text = self.normal_font.render("Credits", True, (255, 255, 255), self.bg_color)
        render_text_rect = render_text.get_rect()
        render_text_rect.center = Vector2(self.surface.get_size()[0] / 2, 300)
        self.surface.blit(render_text, render_text_rect)

        render_text = self.small_font.render(
            "Programers: Andrew Pelton and Tristan Krizan", True, (255, 255, 255), self.bg_color,
        )
        render_text_rect = render_text.get_rect()
        render_text_rect.center = Vector2(self.surface.get_size()[0] / 2, 325)
        self.surface.blit(render_text, render_text_rect)

        render_text = self.small_font.render(
            "Play Testers: Noah", True, (255, 255, 255), self.bg_color
        )
        render_text_rect = render_text.get_rect()
        render_text_rect.center = Vector2(self.surface.get_size()[0] / 2, 350)
        self.surface.blit(render_text, render_text_rect)

        render_text = self.small_font.render(
            "Art: Kenney and Tristan Krizan", True, (255, 255, 255), self.bg_color
        )
        render_text_rect = render_text.get_rect()
        render_text_rect.center = Vector2(self.surface.get_size()[0] / 2, 375)
        self.surface.blit(render_text, render_text_rect)

        render_text = self.normal_font.render(
            "Press SPACE to play Again", True, (255, 255, 255), self.bg_color
        )
        render_text_rect = render_text.get_rect()
